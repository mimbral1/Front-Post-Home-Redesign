  ...
  if (items.length === 0) {
    return (
      <section className="cart-panel empty-cart">
        <h2>Boleta</h2>
        <p>Los productos escaneados apareceran aqui.</p>
      </section>
    );
  }
  ...
  return (
    <section className="cart-panel ticket-panel">
      ...
      <div className="cart-list">
        {items.map((item) => {
          const lineSubtotalBeforeDiscount = item.quantity * item.unitPrice;
          const lineDiscount = lineSubtotalBeforeDiscount - item.total;
          return (
            <article
              key={item.id}
                ...
                <div className="ticket-product">
                  ...
                  {lineDiscount > 0 && (
                    <span className="ticket-discount">
                      Precio {formatCurrency(lineSubtotalBeforeDiscount)} · Desc. -{formatCurrency(lineDiscount)}
                    </span>
                  )}
                </div>
